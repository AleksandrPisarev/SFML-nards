...
void Game::movingChips()
{
    ...
    // проверка есть ли ходы у игрока
    bool possibilityMove = false;
    for (int i = 0; i < 24; ++i) {
        if (moveFromHeadUser == 0 && i == 0)  continue;
        if (field[i].size() != 0 && field[i][0].getColor() == sf::Color::Red) {
            for (int j = 0; j < dice.size(); ++j) {
                if (dice[j].getСondition()) {
                    int pos = i + dice[j].getCircles();
                    if (pos > 23) pos = -1;
                    if (pos >= 0 && (field[pos].size() == 0 || (field[pos].size() != 0 && field[pos][0].getColor() == sf::Color::Red))) {
                        possibilityMove = true;
                        break;
                    }
                }
            }
            if (possibilityMove == true) break;
        }
    }
    /////////////////////////////////////////////
    if (possibilityMove) {
        ...
        ////////// проверяет клик мыши на то место куда необходимо переставить фишку //////////
            ...
            if (firstMoveUser == false && endMove == 4 && dice[0].getCircles() == dice[1].getCircles() && (dice[0].getCircles() == 3 || dice[0].getCircles() == 4 || dice[0].getCircles() == 6)) moveFromHeadUser = 2;
            ...
            for (int i = 0; i < hints.size(); ++i) {
                ...
                if (distance <= 13) {
                    ...
                    for (int j = 0; j < hints[i].getIndexDice().size(); ++j) {
                        if (firstMoveUser == false && dice[0].getCircles() == dice[1].getCircles() && dice[0].getCircles() == 6) {
        ...
        for (int i = 0; i < dice.size(); ++i) {
            if (dice[i].getСondition()) dice[i].noMove();
        }
        Sleep(2000);
        endMove = 0;
    }
